...
vector<pair<int, vector<pair<int, int>>>> taxi_cab_num(int max_results_num, int max_required_pairs_num)
{
    ...
    while (true)
    {
        ...
        for (int x = 1; x < pow(num, 1.0 / 3.0) + 1; x++)
        {
            for (int y = x + 1; y <= pow(num, 1.0 / 3.0) + 1; y++)
            {
                if (x * x * x + y * y * y == num)
                    tmp_pairs.push_back({x, y});
            }
        }
        if (tmp_pairs.size() == max_required_pairs_num)
            results.push_back({num, tmp_pairs});

        if (results.size() == max_results_num)
            break;
        num++;
    }
...
int main()
{
    ...
    vector<pair<int, vector<pair<int, int>>>> ans = taxi_cab_num(2, 2);
